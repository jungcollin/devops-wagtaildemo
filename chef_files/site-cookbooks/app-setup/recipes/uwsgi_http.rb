...
template "/etc/uwsgi/vassals/django-http.ini" do
    user        node["deploy-setup"]["user"]["name"]
    group       node["deploy-setup"]["user"]["group"]
    source      "django-http.ini.erb"
...
file "/var/run/uwsgi-http.pid" do
    user        node["deploy-setup"]["user"]["name"]
    group       node["deploy-setup"]["user"]["group"]
end
...
firewall_rule "uwsgi-http" do
    port        node["app-setup"]["uwsgi_http"]
